...
Item {
    ...
    readonly property string resetLayoutScript: "
        var mainPanel = null, sysTray = null, clock = null, bat = null;
        for (var i = 0; i < panels().length; i++) {
            var p = panels()[i];
            if (p.location === 'bottom') {
                mainPanel = p;
                var t = p.widgets('org.kde.plasma.systemtray');
                if (t.length > 0) sysTray = t[0];
                var c = p.widgets('org.kde.plasma.digitalclock');
                if (c.length > 0) clock = c[0];
                var b = p.widgets('org.kde.plasma.battery');
                if (b.length > 0) bat = b[0];
                break;
            }
        }
        if (sysTray) {
            ...
            sysTray.currentConfigGroup = ['Configuration', 'General'];
            var extra = sysTray.readConfig('extraItems', '').split(',').filter(function(x) { return x !== 'org.kde.plasma.battery' && x !== ''; });
            sysTray.writeConfig('extraItems', extra.join(','));
            var known = sysTray.readConfig('knownItems', '').split(',').filter(function(x) { return x !== 'org.kde.plasma.battery' && x !== ''; });
            sysTray.writeConfig('knownItems', known.join(','));
        }
        if (!bat && mainPanel) bat = mainPanel.addWidget('org.kde.plasma.battery');
        if (bat && clock) bat.index = clock.index;
    "

    MessageDialog {
        id: resetConfirmDialog
        title: qsTr("Reset Layout")
        text: qsTr("Reset panel to BlossomOS default layout?")
        buttons: MessageDialog.Ok | MessageDialog.Cancel
        onAccepted: containment.plasmoid.corona.evaluateScript(editModeItem.resetLayoutScript)
    }
            ...
            RowLayout {
                ...
                Flow {
                    ...
                    PC.ToolButton {
                        ...
                        text: i18ndc("plasma_shell_org.kde.plasma.desktop", "@action:button", "Reset Layout")
